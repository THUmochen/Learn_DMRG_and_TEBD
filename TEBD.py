...
A = np.random.rand(chi, d, chi)#设置初始格点
...
for k in range(1000):
  sigBA_new = ncon([sigBA, *tensors], labels)  # 让A B SAB形成的算符反复缩并到环境张量
  sigBA_new = sigBA_new / np.trace(sigBA_new)  # 归一化
  if LA.norm(sigBA - sigBA_new) < tol:  # 判断是否已经达到收敛
    print('success!')
    break
  else:
    print('iter: %d, diff: %e' % (k, LA.norm(sigBA - sigBA_new)))
    sigBA = sigBA_new#更新矩阵
print(sigBA)
...
sZ = np.array([[1, 0], [0, -1]])
I=np.array([[1, 0], [0, 1]])
hamAB = (( np.kron(sZ, sZ))).reshape(2, 2, 2, 2)#构建local Hamiltonian  +np.kron(sX,I)如何考虑？
hamBA = (( np.kron(sZ, sZ))).reshape(2, 2, 2, 2);
hamX = ((np.kron(sX,I))).reshape(2, 2, 2, 2)

# exponentiate Hamiltonian
tau = 0.01  # set time-step
evotype = 'real'  # set imaginary time evolution
if evotype == "real":
  gateAB = expm(-1j * tau * hamAB.reshape(d**2, d**2)).reshape(d, d, d, d)
  gateX = expm(-1j * tau * hamX.reshape(d**2,d**2)).reshape(d, d, d, d)
...
def doTEBD(hamAB: np.ndarray,
  ...
  muAB = np.eye(A.shape[2]) / A.shape[2]#初始化左右环境张量
...
result=doTEBD(hamAB,hamBA,A,B,sAB,sBA,chi,tau,evotype,1000, 10, 0.0)[0]
print(result)
